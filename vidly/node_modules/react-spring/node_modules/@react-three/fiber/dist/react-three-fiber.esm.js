...
function createRenderer(roots) {
  function applyProps(instance, newProps, oldProps = {}, accumulative = false) {
    ...
    if (filteredPropsEntries.length > 0) {
      ...
      if (handlers.length) {
        if (accumulative && root && instance.raycast) rootState.internal.interaction.push(instance); // Add handlers to the instances handler-map
    ...
    if (child) {
      ...
      if (child.dispose && !child.__r3f.instance) {
        unstable_runWithPriority(unstable_IdlePriority, () => {
          ...
          if (child.dispose && child.type !== 'Scene') child.dispose(); // Remove references
...
function render(element, canvas, {
  ...
  if (!fiber) {
    ...
    store = createStore(applyProps, invalidate, advance, {
      gl: createRendererInstance(gl, canvas),
      size,
      ...props
    });
    const state = store.getState();
    ...
    if (props.vr && gl.xr && gl.setAnimationLoop) {
      gl.xr.enabled = true;
      gl.setAnimationLoop(timestamp => advance(timestamp, true, state));
    }
  }
